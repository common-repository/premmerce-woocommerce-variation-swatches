<?php namespace Premmerce\Attributes\Admin\Tabs;

use Premmerce\SDK\V2\FileManager\FileManager;
use Premmerce\Attributes\Admin\Tabs\Base\TabInterface;
...
class BundleAndSave implements TabInterface
{
    ...
    public function registerBundlesAssets($page)
    {
        if ($page === 'premmerce_page_premmerce-advanced-attributes-admin') {
            ...
            if (isset($_GET['tab']) && $_GET['tab'] === 'bundle_and_save') {
                //css and js for bundle tab
                wp_enqueue_style('premmerce_advanced_attributes_admin_premmerce_style', $this->fileManager->locateAsset('admin/css/premmerce.min.css'));
                wp_enqueue_script(
                    'premmerce_advanced_attributes_admin_bundles_freemius_jquery_for_checkout_script',
                    'https://code.jquery.com/jquery-1.12.4.min.js'
                );
                wp_enqueue_script(
                    'premmerce_advanced_attributes_admin_bundles_freemius_checkout_script',
                    'https://checkout.freemius.com/checkout.min.js'
                );
                wp_enqueue_script(
                    'premmerce_advanced_attributes_admin_bundles_script',
    ...
    {
        return __('Bundle & Save', 'premmerce-advanced-attributes');
    }
